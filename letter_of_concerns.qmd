...
Dear Dr. Silversides and the JACC: Advances editorial team,
...
(i) **Model strategy** ([M1](#appx-m1)): The paper relies on univariable change-score models; field-standard is ANCOVA (follow-up $\sim$ baseline + exposure + covariates).[@vanrosendael2021; @han2020] ANCOVA avoids baseline–change artifacts and yields clearer estimates.[@vickers2001; @tu2007] Omitting age/sex/BP (available in the dataset) leaves associations likely confounded.[@lehman2009; @han2020] No reported 95% CIs for $\beta$. "Predicts" is used for association models without validated prognostic performance.[@moons2015] High baseline–follow-up correlation; univariable $\Delta$-model R² is not decision-relevant; use partial R².
...
**Materials provided:** Appendix (included below); reproducibility GitHub [repository](https://github.com/SloughJE/keto-cta-audit)[@slough_keto_cta_audit_repo]
...
**Data source and variable scope.** CT angiography–derived plaque-burden metrics (PAV, TPS, CAC, NCPV) at baseline and follow-up were obtained from the Citizen Science Foundation keto-CTA repository [@csf_ketocta]. These are the same per-participant plaque metrics used in the publication; summary statistics match the reported values from the study. No other variables (e.g., ApoB, LDL or other lipids, demographics) are available. Analyses are therefore restricted to specifications that rely solely on plaque-burden metrics; models requiring non-plaque variables could not be examined. Per the article's Funding Support, the Citizen Science Foundation funded the study.

**Reproduction materials**. All reproduced figures and analyses are in the [keto-cta-audit](https://github.com/SloughJE/keto-cta-audit) repository with instructions in the README.
...
Similar to Figure 1B, Panel 2F published panel's y-axis label is inconsistent with the plotted scale. The label indicates -1 to 4, but the range should be -1 to 6. The same mislabeling of y-axis ticks is visible in panels **2D** and **2E** (not shown). This can mislead readers about the plotted domain and relative spread.
...
Code for reproduction: `code/02_linear_model_checks.R` ([keto-cta-audit repository](https://github.com/SloughJE/keto-cta-audit))
...
Reproducing the stated univariable linear model $\Delta$TPS $\sim$ CAC$_{bl}$ yields $\beta\ = 0.0017$ (95% CI: −0.0004 to 0.0038), $P = 0.11$, which does not support an association.
...
- For plaque progression, standard practice is ANCOVA (follow-up as a function of baseline and covariates) to obtain clinically interpretable effects at a given starting plaque burden.[@vanrosendael2021; @han2020]
- Primary models omit adjustment for age, sex, and BP despite data availability and known links to plaque progression. These should be included in prespecified covariate sets.
- $\Delta$NCPV is screened against baseline TPS, PAV, CAC, and NCPV without prespecification, even though these predictors are deterministically related and/or highly correlated. For example, TPS is a composite score derived from plaque features, and PAV uses plaque volume relative to vessel volume, so it scales with increases in non-calcified plaque and shares a common denominator across subjects.[@Kishi2016; @Eskerud2021; @Manubolu2024; @Nakanishi2020] This invites mechanical "plaque-predicts-plaque" findings via shared components.
- Total R² (or p-values) from univariable models is not informative about an exposure’s contribution when baseline to follow-up correlation is very high. Specifically, variation in $\mathrm{NCPV}_{\text{follow-up}}$ is dominated by $\mathrm{NCPV}_{\text{baseline}}$ due to within-person persistence (especially after 1 year), so univariable models such as $\Delta\mathrm{NCPV}\sim\mathrm{ApoB}$ or $\mathrm{NCPV}_{\text{follow-up}}\sim\mathrm{ApoB}$ will show low **total** $R^2$ by construction. For example,
$R^2\!\left(\Delta\mathrm{NCPV}\sim\mathrm{NCPV}_{\text{baseline}}\right)=0.49$
while
$R^2\!\left(\mathrm{NCPV}_{\text{follow-up}}\sim\mathrm{NCPV}_{\text{baseline}}\right)=0.96$ (independent re-analysis^[Code for reproduction: `code/02_linear_model_checks.R` ([keto-cta-audit repository](https://github.com/SloughJE/keto-cta-audit))]; not reported in study). Additionally the ICC^[The intraclass correlation coefficient (ICC) is the proportion of total variance attributable to between-participant differences. With two repeated measures per person, ICC can be estimated from a one-way ANOVA (mean squares). A high ICC (0.91) means baseline explains most follow-up variance.] for the follow-up $\sim$ baseline model is 0.91
...
$$
  Interpreting the total R² or significance from a univariable model understates the contribution of ApoB beyond baseline and covariates.[@Cohen2003]
- Multiplicity is not addressed despite many hypothesis tests; without a plan, familywise/type-I error is inflated and false positives are expected.
  - Table 3 has $\sim$19 unique tests; repeating the same panel in two subgroups yields $\sim$57 tests overall. The text references $\Delta$TPS analyses not shown, so the total is plausibly >70.
...
3. **If association is the aim:**
   ...
   - Report partial R² of exposure conditional on baseline (and prespecified covariates)
...
**Issue (mis-specification, inconsistency, interpretation)**
...
Code for reproduction: `code/03_pct_change_demo.R` ([keto-cta-audit repository](https://github.com/SloughJE/keto-cta-audit))
...
This definition differs from the customary per-participant percent change and can under- or over-state the typical effect. The table below illustrates this. It shows the paper's ratio of medians, and the per-subject mean and median percent change.
...
*(4 participants with baseline = 0 excluded from % change calculations for NCPV and PAV)*

**Requested action**

1. Report per-subject percent change with median, IQR and specify how zero or near-zero baseline values were handled when computing percent change.
...
- **Dimensional inconsistency:** the "lifetime" sum mixes **days** and **years** (mg·day/dL **+** mg·year/dL); these are not commensurate.
- **Scale distortion:** the pre-KD term is $\sim$1/365 the magnitude of the day-based terms; if converted to days, the term is a near-linear age surrogate times one LDL value.

**Note.** It appears some Ketogenic Diet Exposure durations are inconsistent across the study.

- Table 1 KD duration: mean **1,642.7 days ($\sim$4.5 y)**.
- Table 3 caption: "LDL-C exposure on a ketogenic diet: mean **5.7 y**."

**Requested action.** Confirm KD duration. Use a single time unit (e.g., convert all time to **days** and label units), report a sensitivity dropping the pre-KD term, and consider also reporting the time-weighted mean LDL alongside total duration.
