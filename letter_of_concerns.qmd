...
(i) **Model strategy** ([M1](#appx-m1)): The paper relies on univariable change-score models; field-standard is ANCOVA (follow-up ∼ baseline + exposure + covariates).[@vanrosendael2021; @han2020] ANCOVA avoids baseline–change artifacts and yields clearer estimates.[@vickers2001; @tu2007] Omitting age/sex/BP (available in the dataset) leaves associations likely confounded.[@lehman2009; @han2020] No reported 95% CIs for $\beta$. "Predicts" is used for association models without validated prognostic performance.[@moons2015] High baseline–follow-up correlation; univariable Δ-model R² is not decision relevant, use partial R².
...
while
$R^2\!\left(\mathrm{NCPV}_{\text{follow-up}}\sim\mathrm{NCPV}_{\text{baseline}}\right)=0.96$ (independent re-analysis; not reported in study). Additionally the ICC^[The intraclass correlation coefficient (ICC) is the proportion of total variance attributable to between-participant differences. With two repeated measures per person, ICC can be estimated from a one-way ANOVA (mean squares). A high ICC (≈0.91) means baseline explains most follow-up variance.] for the follow-up ~ baseline model is 0.91
...
- `regressionBF` used with `rscale = 0.8` and described as "moderately informative."
  - Package named defaults: $\text{medium}=\frac{\sqrt{2}}{4}\approx 0.354 \;|\; \text{wide}=0.5 \;|\; \text{ultrawide}=\frac{\sqrt{2}}{2}\approx 0.707$.[@bayesfactor_regressionbf] 0.8 is not moderate.
...
- **Scale distortion:** the pre-KD term is ~1/365 the magnitude of the day-based terms; if converted to days, the term is a near-linear age surrogate times one LDL value.
