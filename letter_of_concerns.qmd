...
- **Incorrect y-axis tick labels and IQR band** ([C1–C3](#appx-c1)): Fig 1B (change in percent atheroma volume) tick labels show 0–10% while data span 0–20% (twofold underestimation); Fig 1A–B IQR band conflicts with the stated sample IQR (lower bound of band dips <0); Fig 2E–F y-axis should be –1 to 6 (not –1 to 4).
...
**Issue (error).** The shaded IQR band conflicts with the reported sample IQRs.
...
- `regressionBF` run with `rscale = 0.8`, called "moderately informative."
  - Package named defaults: "medium" = 0.354, "wide" = 0.5, "ultrawide" = 0.707; **0.8 is wider than "ultrawide."**[@bayesfactor_regressionbf]
