...
- **Linear-model assumptions violated** ([C5](#appx-c5)): Change-score regressions fail $\ge 2$ diagnostics (e.g., BP $P\le 0.001$; Shapiro–Wilk $P\le 0.001$)
...
##### C4.b — ΔTPS result under the stated lm does not reproduce the paper’s claim.

**Quoted claim (paper).** 

> "(C, F) Only CAC is associated with changes in NCPV and TPS." (p. 7, Figure 2 caption)

Reproducing the stated model (univariable `lm`) does not support an association: $\Delta$TPS $\sim$ CAC$_{bl}$ gives $\beta\ = 0.0017$ (95% CI: −0.0004 to 0.0038), $P = 0.11$.
Additionally:
...
**Requested correction:** amend the TPS claim, or provide supporting analyses with appropriate modeling and full reporting (model specification, coefficients with 95% CIs, and diagnostics).
...
**Requested correction.** Report assumption checks for all fitted models; where violations occur, fit appropriately specified models (e.g., baseline-adjusted follow-up with nonlinear terms; splines/transformations where justified; HC/sandwich SEs) rather than relying solely on robust regression, and include diagnostics with the corrected analyses.
