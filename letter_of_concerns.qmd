...
**Data provenance.** Individual-level plaque metrics used for reproductions were obtained from the Citizen Science Foundation keto-CTA repository (accessed Aug 23, 2025). [@csf_ketocta] The Citizen Science Foundation also funded the study (per the article’s Funding/Disclosures). Basic checks (baseline and change statistics) match the publication.
...
- Primary reliance on univariable change-score models invites confounding (e.g., age, sex) and mathematical coupling with baseline[@tu2007; @clifton2019]; change-scores compound measurement error and reduce power. Baseline imbalance and regression-to-the-mean can inflate, attenuate, or reverse associations.[@chiolero2013]
